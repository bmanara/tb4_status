...
class TB4StatusNode(Node):
    ...
    def __init__(self):
        ...
        self.dock_status_subscriber = self.create_subscription(
            DockStatus,
            'dock_status',
    ...
    def dock_status_callback(self, msg: DockStatus):
        ...
        if dock_status and not self.is_docked: # If the robot just docked
            self.is_docked = True
            self.get_logger().info("Robot is now docked.")
        elif not dock_status and self.is_docked: # If the robot just undocked
            self.is_docked = False
            self.get_logger().info("Robot is now undocked.")
